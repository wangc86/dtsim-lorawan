...
/* Description: This code takes those generated by sensor-topology-gen.cpp
*              as input data, and creates the necessary data for dtsim.cpp.
*              Given each sensor topology, it sets up a random initial time
*              offset (i.e., phase) for each sensor node.
*              Here we also define where those static gateways are,
*              and by which we may compute the needed SF values (i.e.,
*              spreading factor, LoRa's design parameter) for each sensor.
*/
...
{
    // Define the locations of static gateways
    int gwx[25], gwy[25];
    for (int i = 0; i < 25; i++)
    {
        gwx[i] = 125 + 250*(i%5);
    ...
    // and therefore there should be no outliers.
